...
public class NPC extends cn.nukkit.level.Location {
		...
		for (String key : NPC.unknownTypeData.keySet()) {
			...
			if (npc != null) {
				try {
					...
					Main.getInstance().getLogger().warning(TextFormat.YELLOW + "鍔犺浇ID涓� " + TextFormat.AQUA + key
							+ TextFormat.YELLOW + " 鐨凬PC鏃跺嚭鐜颁簡鏈煡閿欒");
				}
				remove.add(key);
			}
		}
	...
	public static void init() {
		if (Main.getInstance().getDataFolder().mkdirs()
			...
			Main.getInstance().getLogger().debug("閰嶇疆鏂囦欢宸插垱寤�");
		}
	...
	public NPC(String nid, String nametag, double x, double y, double z, Item handItem) {
		...
		if (existsCheck != null) {
			Main.getInstance().getLogger().warning(TextFormat.YELLOW + "璀﹀憡:灏濊瘯鍒涘缓ID閲嶅NPC:" + TextFormat.AQUA + nid
					+ TextFormat.YELLOW + " ,璇锋鏌ユ槸鍚﹀嚭鐜伴�昏緫閿欒");
			existsCheck.close();
		}
	...
	public boolean spawnTo(Player player, Level level) {
		...
		pk.metadata = new EntityMetadata().putByte(Entity.DATA_FLAGS, 0).putShort(Entity.DATA_AIR, 300)
				.putString(Entity.DATA_NAMETAG, this.nametag).putByte(Entity.DATA_SHOW_NAMETAG, 1)
				.putByte(Entity.DATA_SILENT, 0).putByte(Entity.DATA_NO_AI, 0);
		player.dataPacket(pk);
	...
	public void sendPosition() {
		...
		pk.headYaw = Utils.cast(this.yaw);
